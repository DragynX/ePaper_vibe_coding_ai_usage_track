...
bool MiniMaxUsageClient::fetch(long now, ProviderQuota& out) {
  ...
  if (ar.http.status != 200) {
    Serial1.printf("[minimax/usage] status %d\n", ar.http.status);
    return false;
  }

  JsonDocument doc;
  if (deserializeJson(doc, ar.http.body)) {
    Serial1.println("[minimax/usage] json parse error");
    return false;
  }
  if ((doc["base_resp"]["status_code"] | -1) != 0) {
    Serial1.println("[minimax/usage] API error");
    return false;
  }

  JsonArray models = doc["model_remains"];
  if (models.isNull() || models.size() == 0) {
    Serial1.println("[minimax/usage] no model_remains");
    return false;
  }
