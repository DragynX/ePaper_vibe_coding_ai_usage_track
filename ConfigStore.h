...
class ConfigStore {
 public:
  ...
  int  uiWeight() const         { return ui_weight_; }
  bool uiSmallCrisp() const     { return ui_small_crisp_; }
  ...
  void setUiWeight(int v)                { ui_weight_ = v; }
  void setUiSmallCrisp(bool v)           { ui_small_crisp_ = v; }
 ...
 private:
  ...
  bool    dark_        = false;
  int     ui_font_     = 4;       // selected device typeface index (0..N-1); 4 = Lexend
  bool    ui_aa_       = false;   // grayscale AA (smooth) vs crisp 1-bit; default crisp (Hard Black on)
