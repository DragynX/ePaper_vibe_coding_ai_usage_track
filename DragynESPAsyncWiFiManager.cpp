...
/* v4.7.144 (WPA3 audit): forward decl of helper defined in DragynWeather.ino;
 * called below before WiFi.begin(ssid,pass) so the portal-saved credentials
 * persist with PMF-capable + SAE_PWE_BOTH set on the wifi_config_t. */
extern void dragynWifiApplyWpa3Hardening(void);
...
{
  // 4.7.151 — APITips: disable modem-sleep while the captive portal is up so
  // beacons are emitted reliably while users are joining (default modem-sleep
  // can briefly suspend the radio between DTIM beacons and cause some phones
  // to drop the AP from their list). Restored at the end of the blocking
  // portal call (see startConfigPortal()).
  WiFi.setSleep(false);
  ...
  // 4.7.151 — APITips: lock channel to 1 (one of 1/6/11 — non-overlapping
  // 2.4 GHz channels reduce neighbor-AP interference during onboarding); pass
  // ssid_hidden=0 (broadcast SSID; user-friendly) and max_connection=4
  // (Arduino default, made explicit so future core changes can't silently
  // change behavior).
  if (_apPassword != NULL)
  {
    WiFi.softAP(_apName, _apPassword, /*channel=*/1, /*ssid_hidden=*/0, /*max_connection=*/4); // password option
  }
  else
  {
    WiFi.softAP(_apName, /*password=*/NULL, /*channel=*/1, /*ssid_hidden=*/0, /*max_connection=*/4);
  }

  delay(500); // without delay I've seen the IP address blank
  // 4.7.151 — APITips: softAP can silently fail to bring up the netif on the
  // first call (rare, seen during heavy boot-time WiFi churn). Detect a
  // 0.0.0.0 AP IP and re-issue softAP() once before proceeding so DHCP/DNS
  // come up reliably.
  if (WiFi.softAPIP() == IPAddress(0, 0, 0, 0))
  {
    DEBUG_WM(F("softAP IP came back 0.0.0.0; retrying once"));
    if (_apPassword != NULL)
    {
      WiFi.softAP(_apName, _apPassword, /*channel=*/1, /*ssid_hidden=*/0, /*max_connection=*/4);
    }
    else
    {
      WiFi.softAP(_apName, /*password=*/NULL, /*channel=*/1, /*ssid_hidden=*/0, /*max_connection=*/4);
  ...
  // 4.7.151 — APITips: always-on AP boot diagnostic. Captures the four
  // settings that captive-portal interop hinges on; cheap line for log bisect.
  Serial.printf("[AP] SSID=%s channel=1 ip=%s max_conn=4 hidden=0\n",
                _apName ? _apName : "(null)",
...
// setup the DNS server redirecting all the domains to the apIP
  ...
  // 4.7.151 — APITips: OS captive-portal probe URLs. Bind directly to handleWifi
  // so the response is 200 + HTML (the SSID picker), not the captivePortal() 302.
  // Some Android/Samsung variants chase the 302 silently and miss the popup; an
  // explicit 200 with a non-success body is the most reliable trigger for the
  // captive-portal sheet across Android/iOS/Windows.
  ...
  server->on("/gen_204",
             std::bind(&AsyncWiFiManager::handleWifi, this, std::placeholders::_1, true))
      .setFilter(ON_AP_FILTER);
  // ---- Apple iOS / macOS probes (expects body "Success"; returning the SSID
  // picker HTML is "not Success" and triggers the captive-portal sheet):
  ...
  server->on("/library/test/success.html",
             std::bind(&AsyncWiFiManager::handleWifi, this, std::placeholders::_1, true))
      .setFilter(ON_AP_FILTER);
  // ---- Microsoft Windows 10/11 probes (NCSI = Network Connectivity Status
  // Indicator; expects body "Microsoft Connect Test" / "Microsoft NCSI" —
  // returning HTML triggers the "Action needed — sign in to network" toast):
  ...
  server->on("/ncsi.txt",
             std::bind(&AsyncWiFiManager::handleWifi, this, std::placeholders::_1, true))
      .setFilter(ON_AP_FILTER);
  // ---- Edge clients: Firefox (canonical.html), Samsung / older-Android
  // (success.txt), Windows DHCP WPAD auto-discovery (wpad.dat), generic
  // /redirect path used by some captive-portal libs. All return the SSID
  // picker HTML so any unrecognized probe still gets the user to the form.
...
void AsyncWiFiManager::copySSIDInfo(wifi_ssid_count_t n)
{
  ...
  if (n > 0)
  {
    ...
    /* DragynWeather: reverse list so the strongest RSSI appears at the bottom of the portal dropdown. */
    for (int i = 0; i < (int)n / 2; i++)
    {
      std::swap(wifiSSIDs[i], wifiSSIDs[(int)n - 1 - i]);
    }
  }
}
  ...
  if (_modeless)
  {
    ...
    if (connect)
    {
      ...
      if (connectWifi(_ssid, _pass) != WL_CONNECTED)
      {
        ...
        if (_portalStaConnectFailedCallback)
        {
          _portalStaConnectFailedCallback();
        }
      }
      else
      {
        ...
        {
          (void)esp_wifi_set_config(WIFI_IF_STA, &wcfg);
        }
#else
        WiFi.persistent(true);
#endif
...
// DNS
    ...
    if (connect)
    {
      ...
      /* Do not write STA credentials to NVS until association succeeds (ESP WiFi NVS). */
      WiFi.persistent(false);
      const uint8_t connRes = connectWifi(_ssid, _pass);
      if (connRes == WL_CONNECTED)
      {
#if !defined(ESP8266)
        /* Commit current STA profile to flash now that connection is verified. */
        WiFi.persistent(true);
        wifi_config_t wcfg;
        memset(&wcfg, 0, sizeof(wcfg));
        if (esp_wifi_get_config(WIFI_IF_STA, &wcfg) == ESP_OK)
  ...
  // 4.7.151 — APITips: restore modem-sleep (Arduino default) now that the
  // captive portal has exited. setSleep(false) was set in setupConfigPortal()
  // for beacon reliability during onboarding.
  WiFi.setSleep(true);
...
uint8_t AsyncWiFiManager::connectWifi(String ssid, String pass)
{
  ...
  if (ssid != "")
  {
    ...
    WiFi.disconnect(false);
#endif
    dragynWifiApplyWpa3Hardening();  /* v4.7.144 (WPA3 audit) */
...
void AsyncWiFiManager::handleRoot(AsyncWebServerRequest *request)
{
  ...
  {
    // OS captive-portal probe: redirect to the AP IP first
    return;
  }
  // Skip the legacy "Options" landing page; serve the SSID picker as root.
...
void AsyncWiFiManager::handleWifiSave(AsyncWebServerRequest *request)
{
  ...
  if (_ssid.length() == 0)
  {
    DEBUG_WM(F("WiFi save rejected: empty SSID"));
    String err = FPSTR(WFM_HTTP_HEAD);
    err.replace("{v}", "Wi-Fi");
    err += FPSTR(HTTP_SCRIPT);
    err += FPSTR(HTTP_STYLE);
    err += _customHeadElement;
    err += FPSTR(HTTP_HEAD_END);
    err += F("<p>Select or enter a network name (SSID), then tap save.</p>");
    err += F("<p><a href=\"/wifi\">Back to Wi-Fi setup</a></p>");
    err += FPSTR(HTTP_END);
    request->send(400, "text/html", err);
    return;
  }

  if (_portalWifiSaveCallback)
  {
    _portalWifiSaveCallback(_ssid.c_str());
  }
...
void AsyncWiFiManager::setPortalWifiSaveCallback(std::function<void(const char *ssid)> func)
{
  _portalWifiSaveCallback = func;
}

void AsyncWiFiManager::setPortalStaConnectFailedCallback(std::function<void()> func)
{
  _portalStaConnectFailedCallback = func;
}
