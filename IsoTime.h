...
// Claude access token: expired if within a 5-minute buffer of expiry.
// epoch=0 means "not set by user" — treat as valid and let the 401 path handle it.
inline bool umClaudeTokenExpired(long expiryEpochSec, long nowEpoch) {
  if (expiryEpochSec <= 0) return false;
