...
#ifndef DragynESPAsyncWiFiManager_h
#define DragynESPAsyncWiFiManager_h
...
public:
  ...
  /** Called as soon as user submits /wifisave (before STA connect attempt). For TFT feedback. */
  void setPortalWifiSaveCallback(std::function<void(const char *ssid)> func);
  /** Called when user-submitted STA credentials fail to associate (portal still up). */
  void setPortalStaConnectFailedCallback(std::function<void()> func);
...
private:
  ...
  boolean connect;
  /* Default off in production builds to reduce serial noise; enable explicitly when troubleshooting portal flow. */
  ...
  std::function<void(const char *ssid)> _portalWifiSaveCallback;
  std::function<void()> _portalStaConnectFailedCallback;
